...
public class PostWeapon extends DeclarativeWebScript {
	...
	protected Map<String, Object> executeImpl(WebScriptRequest req, Status status) {
		...
		if (name == null || type == null || ammo == null || fireMode == null || isBlocked == null || 
			name.isEmpty() || type.isEmpty() || ammo.isEmpty() || fireMode.isEmpty() || isBlocked.isEmpty()) {
		...
		if (dropzoneFolder == null) {
			...
			String concatProps = String.join("_", GameModel.TYPE_G_WEAPON.getLocalName(), name, type, ammo, fireMode, isBlocked, skinName);
			NodeRef newWeapon = fileFolderService.create(dropzoneFolder, concatProps, ContentModel.TYPE_CONTENT).getNodeRef();
